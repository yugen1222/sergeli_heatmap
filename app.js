...
async function unlockVoice(){
  const button = document.getElementById("voiceUnlockBtn");
  try{
    button.disabled = true;
    button.textContent = "⏳ Проверка звука...";

    // Проверяем, что MP3 реально доступен на Render/GitHub.
    const response = await fetch("./floor1.mp3", {cache:"no-store"});
    if(!response.ok){
      throw new Error(`floor1.mp3 не найден: HTTP ${response.status}`);
    }

    // Первый запуск обязан происходить непосредственно после нажатия пользователя.
    const audio = floorVoice[1];
    audio.pause();
    audio.currentTime = 0;
    audio.volume = 1;
    audio.muted = false;
    ...
    voiceUnlocked = true;
    button.textContent = "🔊 Голос включён";

    // После короткого теста не обрываем запись сразу: пользователь должен услышать её.
    audio.onended = () => {
      isVoicePlaying = false;
      const red = [1,2,3].filter(i => floors[i] && getStatus(floors[i]) === "red");
      red.forEach(i => addVoiceToQueue(i, true));
    };

    isVoicePlaying = true;
  }catch(e){
    console.error("Ошибка голосового уведомления:", e);
    voiceUnlocked = false;
    button.textContent = "🔊 Включить голос";
    alert(
      "Звук не запустился.\n\n" +
      "1. Проверьте громкость устройства.\n" +
      "2. Разрешите звук для сайта в настройках браузера.\n" +
      "3. Проверьте, что floor1.mp3 открывается по ссылке сайта.\n\n" +
      "Ошибка: " + (e?.message || e)
    );
  }finally{
...
async function testVoice(floor){
  try{
    ...
    // Тест запускается непосредственно от клика и поэтому не блокируется браузером.
    voiceQueue = [];
    Object.values(floorVoice).forEach(a => {
      a.pause();
      a.currentTime = 0;
      a.volume = 1;
      a.muted = false;
    ...
    const response = await fetch(`./floor${floor}.mp3`, {cache:"no-store"});
    if(!response.ok) throw new Error(`floor${floor}.mp3 не найден: HTTP ${response.status}`);

    isVoicePlaying = true;
    lastVoiceTime[floor] = Date.now();
    audio.onended = () => {
      isVoicePlaying = false;
      setTimeout(playNextVoice, 800);
    };
    await audio.play();
  }catch(e){
    isVoicePlaying = false;
    console.error(e);
    alert("Не удалось воспроизвести floor" + floor + ".mp3: " + (e?.message || e));
  }
}
...
document.getElementById("voiceUnlockBtn").textContent="🔊 Включить голос";
render();
renderRequests();
